...
rollout (before an inapplicable step, or the terminal state if every planned step
succeeded—including when the first action fails, in which case carry stays at that
iteration's rollout start).
If there is **no plan**, carry is cleared and planning uses the PDDL initial again.

Post-loop sound check still uses the **original** PDDL initial (``gt_problem``) to
produce results that are comparable across runs.

This is the **no-restart** counterpart to ``loop_ascal_restart.py``, which resets
to the PDDL initial state upon any negative demo.

**Smoke test:** ``python scripts/loop_ascal_no_restart.py --domain blocks --problem problem-00 --max-outer 1``

Default benchmark root is ``<repo>/benchmarks`` (override with ``--benchmarks-root``).
Example: ``--domain blocks --problem problems/problem-00.pddl`` loads
``benchmarks/blocks/domain_original.pddl`` when present. Legacy tree:
``--benchmarks-root <repo>/benchmarks`` (e.g. ``debug_pq`` lives there only).
...
**Server log file:** by default writes to ``<exp_root>/results/results_ascal_no_restart/<domain>/<problem-stem>.log``
...
- ``MAX_OUTER_ITERATIONS`` must be an int (``None`` breaks ``while (i+1) < ...``).
- Simulators return ``(pos_demos, neg_demo, final_up_state, carry_up_state)``; the last
  field is always the last valid UP state after the walk (same as ``final_up_state`` when
  the full prefix runs). The main loop still clears carry when ``plan is None``.
- Verification / goal check / ``lift_demonstrations`` batch match the notebook.
- No undefined ``demo``, ``pos_demos``, ``QUIET_ONLINE_LOOP``, ``_n_init``.

No-plan behavior: skip learning for that outer iteration (same as switch notebook without
random-walk fallback). Add your own fallback below the ``if plan is None`` branch if needed.
...
for both preconditions and effects). No sound/complete UP models are built.
"""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        metavar="PATH",
        help=(
            "Log path (default: <exp_root>/results/results_ascal_no_restart/<domain>/<problem-stem>.log)."
        ),
    )
    ap.add_argument(
        "--no-log-file",
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "(default: <exp_root>/results/results_ascal_no_restart)."
        ),
    )
    args = ap.parse_args()
    ...
    results_root = (
        ...
        else experiment_root / "results" / "results_ascal_no_restart"
    )
    default_log_file = (
    ...
    try:
        if log_file_path is not None:
            ...
            print(f"[loop_ascal_no_restart] log file: {log_file_path}", file=sys.stderr)
        ...
        with OneshotPlanner(name="fast-downward", problem_kind=plan_problem.kind) as planner:
            while (i + 1) < MAX_OUTER_ITERATIONS:
                ...
                carry_up_state = sim_carry_state if plan is not None else None
