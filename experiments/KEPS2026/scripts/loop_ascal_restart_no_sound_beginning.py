...
**Smoke test:**
    ...
    python scripts/loop_ascal_restart_no_sound_beginning.py --domain blocks --problem problem-00 --max-outer 1
...
**Server log file:** by default writes to ``<exp_root>/results/results_ascal_restart_no_sound_beginning/<domain>/<problem-stem>.log``
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        metavar="PATH",
        help=(
            "Log path (default: <exp_root>/results/results_ascal_restart_no_sound_beginning/"
            "<domain>/<problem-stem>.log)."
        ),
    )
    ap.add_argument(
        "--no-log-file",
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "(default: <exp_root>/results/results_ascal_restart_no_sound_beginning)."
        ),
    )
    args = ap.parse_args()
    ...
    results_root = (
        ...
        else experiment_root / "results" / "results_ascal_restart_no_sound_beginning"
    )
    default_log_file = (
    ...
    try:
        if log_file_path is not None:
            ...
            print(
                f"[loop_ascal_restart_no_sound_beginning] log file: {log_file_path}",
                file=sys.stderr,
            )
