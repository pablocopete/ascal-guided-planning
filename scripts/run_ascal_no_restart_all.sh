...
# Logs go to <repo>/results_ascal_no_restart/<domain>/ by default.
#
# Usage:
#   bash scripts/run_ascal_no_restart_all.sh
#   bash scripts/run_ascal_no_restart_all.sh 8
#   MAX_JOBS=8 bash scripts/run_ascal_no_restart_all.sh
#
# Optional: RESULTS_ROOT=/path/to/logs bash scripts/run_ascal_no_restart_all.sh
...
SCRIPT="$ROOT/scripts/loop_ascal_no_restart.py"
BENCH_ROOT="$ROOT/benchmarks"
RESULTS_ROOT="${RESULTS_ROOT:-$ROOT/results_ascal_no_restart}"
...
echo "[run_ascal_no_restart_all] repo=$ROOT benchmarks=$BENCH_ROOT results=$RESULTS_ROOT max_concurrent_jobs=$MAX_JOBS total_tasks=${#COMMANDS[@]}" >&2
